...
const AreaTable = (props) => {
    ...
    const{data,
        ...
        loadingchange,
        sortHandler,
        handleChange,itemperpageHandler} = Tablehook("areas",searchh,["id","name",'businessunit.name',"status"],searchterm,props.search);
    const searchClick = (n) => {
        setSearchterm(n);
    }            
            ...
            <div className="table-responsive">
                <table className="table align-items-center mb-2">
                    <thead className="table-dark">
                        <tr>
                            <th className="text-center text-xs font-weight-bold">Area&nbsp;<span data-sort="name" onClick={()=>sortHandler("name")}><ArrowDownUp size={15}/></span>&nbsp;<span onClick={()=>{handleSearchInputs(); searchClick("name")}}><Funnel size={15}/></span></th>
