...
const PresalesItem = (props) => {   
    ...
    var link = 'http://localhost:8000/api/presales/'+props.id+'/'+fct;
    ...
    if(disabledValue){
        message = "This Presales Engineer is used you can't deactivate it.";
    }
    else{
        if(props.status)
            message = "Deactivate";
    ...
    return(
        <tr>
            <td key={Math.random().toString(36).substr(2, 9)} className="align-middle text-center text-sm">
                {authctx.updatePresalesEngineers && <h4 className="title"><Link to={`/administration/presales/edit/${props.id}`}>{props.name}</Link></h4>}
                {!authctx.updatePresalesEngineers && <h4>{props.name}</h4>}
            </td>
            <td key={Math.random().toString(36).substr(2, 9)} className="align-middle text-center text-sm">
            ...
            <td key={Math.random().toString(36).substr(2, 9)} className="align-middle text-center text-sm">
                <h4>{delvergule(props.areaName)}</h4>
            </td>
            <td key={Math.random().toString(36).substr(2, 9)} className="align-middle text-center text-sm">
